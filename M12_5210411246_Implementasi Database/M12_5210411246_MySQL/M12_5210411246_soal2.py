...
def search(kd_mk):
    try:
        cur.execute(
            "SELECT nama_mk, sks, semester, waktu, tempat, `nama_dos` FROM `kuliah`,  `mata kuliah`, `dosen` WHERE `kuliah`.`kode_mk`=`mata kuliah`.`kode_mk` AND `kuliah`.`kode_dos`=`dosen`.`kode_dos` AND `mata kuliah`.`kode_mk`= %s", [kd_mk])
...
def insert():
    try:
        ...
        cur.execute("INSERT INTO dosen (kode_dos, nama_dos, alamat_dos, no_tlp) VALUES (%s,%s,%s,%s)", [
                    kode_dos, nama_dos, alamat_dos, no_tlp])
        connect.commit()
        cur.execute("INSERT INTO `mata kuliah` (kode_mk, nama_mk, sks, semester) VALUES (%s,%s,%s,%s)", [
                    kode_mk, nama_mk, sks, semester])
        connect.commit()
        cur.execute("INSERT INTO `kuliah` (kode_mk, kode_dos, waktu, tempat) VALUES (%s,%s,%s,%s)", [
...
def update(kd_mk):
    cur.execute(
        "SELECT kuliah.kode_mk, nama_mk, sks, semester, `nama_dos` FROM `kuliah`,  `mata kuliah`, `dosen` WHERE `kuliah`.`kode_mk`=`mata kuliah`.`kode_mk` AND `kuliah`.`kode_dos`=`dosen`.`kode_dos` AND `mata kuliah`.`kode_mk`= %s", [kd_mk])
    try:
        for x in cur.fetchall():
            if kd_mk in x[0]:
                print("\n==============================================")
                ...
                cur.execute("UPDATE kuliah SET waktu = %s, tempat = %s WHERE kode_mk LIKE %s", [
                            waktu, tempat, kd_mk])
                connect.commit()
...
def delete(kd_mk):
    cur.execute(
        "SELECT kuliah.kode_mk, dosen.kode_dos FROM `kuliah`, `dosen` WHERE `kuliah`.`kode_dos`=`dosen`.`kode_dos` AND `kuliah`.`kode_mk`= %s", [kd_mk])
    try:
        for x in cur.fetchall():
            kode_mk = x[0]
            kode_dos = x[1]
            if kd_mk in x[0]:
                cur.execute(
                    "DELETE FROM `kuliah` WHERE `kode_mk`= %s", [kode_mk])
                connect.commit()
                cur.execute(
                    "DELETE FROM `mata kuliah` WHERE `kode_mk`= %s", [kode_mk])
                connect.commit()
                cur.execute(
                    "DELETE FROM `dosen` WHERE `kode_dos`= %s", [kode_dos])
                connect.commit()
