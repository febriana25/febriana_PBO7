...
def select_nasabah(no_rekening):
    try:
        cur.execute("SELECT no_rekening,  nasabah.id_nasabah ,nama_nasabah, saldo, nama_cabang FROM nasabah, rekening, cabang_bank WHERE nasabah.id_nasabah=rekening.id_nasabah AND cabang_bank.kode_cabang=rekening.kode_cabang AND rekening.no_rekening LIKE %s", [
                    no_rekening])
...
def insert_nasabah():
    try:
        ...
        cur.execute("INSERT INTO cabang_bank (kode_cabang, nama_cabang, alamat_cabang) VALUES (%s,%s,%s)", [
        ...
        cur.execute("INSERT INTO nasabah (id_nasabah, nama_nasabah, alamat_nasabah) VALUES (%s,%s,%s)", [
        ...
        cur.execute("INSERT INTO rekening (no_rekening, pin, saldo, id_nasabah, kode_cabang) VALUES (%s,%s,%s,%s,%s)", [
    ...
    try:
        for x in cur.fetchall():
            if id_nasabah == x[0]:
                ...
                cur.execute("UPDATE rekening SET pin = %s WHERE id_nasabah LIKE %s", [
                            pin, id_nasabah])
                connect.commit()
                print("\nUpdate Berhasil")
            kembali()
...
def transaksi(no_rekening):
    cur.execute("SELECT no_rekening ,nasabah.id_nasabah, nama_nasabah ,saldo FROM nasabah, rekening WHERE nasabah.id_nasabah=rekening.id_nasabah AND rekening.no_rekening LIKE %s", [
                no_rekening])
    try:
        for x in cur.fetchall():
            id_nasabah = x[1]
            ...
            if no_rekening in x[0]:
                ...
                if menu == '1':
                    ...
                    cur.execute("UPDATE rekening SET saldo = %s WHERE no_rekening LIKE %s", [
                                hasil, no_rekening])
                    connect.commit()
                    cur.execute("INSERT INTO transaksi (no_transaksi, jenis_nasabah, tanggal, jumlah, id_nasabah) VALUES (%s,%s,%s,%s,%s)", [
...
def select_transaksi(id_nasabah):
    cur.execute(
        "SELECT * FROM transaksi WHERE id_nasabah LIKE %s", [id_nasabah])
...
def delete(id_nasabah):
    cur.execute(
        "SELECT nasabah.id_nasabah, no_rekening, kode_cabang, nama_nasabah FROM rekening, nasabah WHERE rekening.id_nasabah=nasabah.id_nasabah AND rekening.id_nasabah LIKE %s", [id_nasabah])
    try:
        for x in cur.fetchall():
            no_rekening = x[1]
            kode_cabang = x[2]
            if id_nasabah in x[0]:
                cur.execute(
                    "DELETE FROM nasabah WHERE id_nasabah = %s", [id_nasabah])
                connect.commit()
                cur.execute(
                    "DELETE FROM rekening WHERE no_rekening = %s", [no_rekening])
                connect.commit()
                cur.execute(
                    "DELETE FROM transaksi WHERE id_nasabah = %s", [id_nasabah])
                connect.commit()
                cur.execute(
                    "DELETE FROM cabang_bank WHERE kode_cabang = %s", [kode_cabang])
                connect.commit()
