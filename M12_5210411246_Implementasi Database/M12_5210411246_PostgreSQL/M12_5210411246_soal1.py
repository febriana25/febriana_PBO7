...
class Jabatan:
    ...
    def search(kd_jabatan):
        try:
            cur.execute(
                "SELECT * FROM jabatan  WHERE kode_jabatan LIKE %s", [kd_jabatan])
        ...
        try:
            for x in cur.fetchall():
                if kd_jabatan in x[0]:
                    ...
                    sql = (
                        "UPDATE public.jabatan SET gapok = %s, tunjangan_jabatan = %s WHERE kode_jabatan LIKE %s")
    ...
    def delete(kd_jabatan):
        ...
        try:
            for x in cur.fetchall():
                if kd_jabatan in x[0]:
                    cur.execute(
                        "DELETE FROM jabatan WHERE kode_jabatan LIKE %s", [kd_jabatan])
                    connect.commit()
        ...
        try:
            for x in cur.fetchall():
                if nip in x[0]:
                    ...
                    sql = (
                        "UPDATE pegawai SET kode_golongan = %s, status = %s, jumlah_anak = %s WHERE nip LIKE %s")
        ...
        try:
            for x in cur.fetchall():
                if nip in x[1]:
                    ...
                    potongan = int(input("Masukan Potongan\t: "))
                    sql = (
                        "UPDATE gaji SET masuk = %s, sakit = %s, izin = %s, alfa = %s, lembur = %s, potongan = %s WHERE nip LIKE %s")
...
class Golongan:
    ...
    def search(nm_golongan):
        try:
            cur.execute(
                "SELECT * FROM golongan  WHERE nama_golongan LIKE %s", [nm_golongan])
        ...
        try:
            for x in cur.fetchall():
                if nm_golongan in x[1]:
                    ...
                    sql = (
                        "UPDATE golongan SET tunjangan_suami = %s, tunjangan_anak = %s, uang_makan = %s, uang_lembur = %s WHERE nama_golongan LIKE %s")
    ...
    def delete(nm_golongan):
        ...
        try:
            for x in cur.fetchall():
                if nm_golongan in x[1]:
                    cur.execute(
                        "DELETE FROM golongan WHERE nama_golongan LIKE %s", [nm_golongan])
