...
# CREATE TABLE
# cur.execute(
#     "CREATE TABLE public.supplier (kode_sup text DEFAULT 3, nama_sup text DEFAULT 30)")
# cur.execute(
#     "CREATE TABLE public.barang (kode_brg text DEFAULT 3, nama_brg text DEFAULT 30)")
...
def search(kd_brg):
    try:
        cur.execute(
            "SELECT nama_sup ,nama_brg, tanggal ,tempo, qty, harga, total FROM supplier, barang, transaksi_brg, nota WHERE supplier.kode_sup=nota.kode_sup AND barang.kode_brg=transaksi_brg.kode_brg AND nota.no_nota=transaksi_brg.no_nota AND barang.kode_brg=%s", [kd_brg])
...
def insert():
    try:
        ...
        total = qty * harga
        cur.execute("INSERT INTO supplier (kode_sup, nama_sup) VALUES (%s,%s)", [
                    kode_sup, nama_sup])
        connect.commit()
        cur.execute("INSERT INTO barang (kode_brg, nama_brg) VALUES (%s,%s)", [
                    kode_brg, nama_brg])
        connect.commit()
        cur.execute("INSERT INTO nota (no_nota, tanggal, tempo, total, kode_sup) VALUES (%s,%s,%s,%s,%s)", [
                    no_nota, tanggal, tempo, total, kode_sup])
        connect.commit()
        cur.execute("INSERT INTO transaksi_brg (no_nota, kode_brg, qty, harga) VALUES (%s,%s,%s,%s)", [
                    no_nota, kode_brg, qty, harga])
        connect.commit()
...
def update(kd_brg):
    cur.execute(
        "SELECT barang.kode_brg, nota.kode_sup ,nama_sup ,nama_brg, tanggal ,tempo, qty, harga, total FROM supplier, barang, transaksi_brg, nota WHERE supplier.kode_sup=nota.kode_sup AND barang.kode_brg=transaksi_brg.kode_brg AND nota.no_nota=transaksi_brg.no_nota AND barang.kode_brg=%s", [kd_brg])
    try:
        for x in cur.fetchall():
            kode_brg = x[0]
            kode_sup = x[1]
            if kd_brg in x[0]:
                ...
                total = qty * harga
                cur.execute("UPDATE transaksi_brg SET qty = %s, harga = %s WHERE kode_brg LIKE %s", [
                            qty, harga, kode_brg])
                connect.commit()
                cur.execute("UPDATE nota SET total = %s WHERE kode_sup LIKE %s", [
                            total, kode_sup])
                connect.commit()
...
def delete(kd_brg):
    ...
    try:
        for x in cur.fetchall():
            ...
            if kd_brg in x[0]:
                cur.execute(
                    "DELETE FROM barang WHERE kode_brg = %s", [kode_brg])
                connect.commit()
                cur.execute(
                    "DELETE FROM supplier WHERE kode_sup = %s", [kode_sup])
                connect.commit()
                cur.execute("DELETE FROM nota WHERE kode_sup = %s", [kode_sup])
                connect.commit()
                cur.execute(
                    "DELETE FROM transaksi_brg WHERE kode_brg = %s", [kode_brg])
                connect.commit()
