...
class Jabatan:
    ...
    def search(kd_jabatan):
        # try :
        cur.execute(
            "SELECT * FROM jabatan  WHERE kode_jabatan LIKE ?", [kd_jabatan])
    ...
    def update(kd_jabatan):
        cur.execute(
            f"SELECT * FROM jabatan WHERE kode_jabatan LIKE ?", [kd_jabatan])
        try:
            for x in cur.fetchall():
                if kd_jabatan in x[0]:
                    ...
                    sql = (
                        "UPDATE jabatan SET gapok = ?, tunjangan_jabatan = ? WHERE kode_jabatan LIKE ?")
    ...
    def delete(kd_jabatan):
        ...
        try:
            for x in cur.fetchall():
                if kd_jabatan in x[0]:
                    cur.execute(
                        f"DELETE FROM jabatan WHERE kode_jabatan LIKE ?", [kd_jabatan])
                    connect.commit()
...
class Pegawai:
    ...
    def insert():
        try:
            ...
            data = (nip, nm_pegawai, kd_jabatan,
                    kd_golongan, status, jmlh_anak)
            cur.execute(sql, data)
            connect.commit()
            print("\nData Berhasil Di Tambahkan")
            kembali()
        ...
        try:
            for x in cur.fetchall():
                if nip in x[0]:
                    ...
                    sql = (
                        "UPDATE pegawai SET kode_golongan = ?, status = ?, jumlah_anak = ? WHERE nip LIKE ?")
        ...
        try:
            for x in cur.fetchall():
                if nip in x[1]:
                    ...
                    potongan = int(input("Masukan Potongan\t: "))
                    sql = (
                        "UPDATE gaji SET masuk = ?, sakit = ?, izin = ?, alfa = ?, lembur = ?, potongan = ? WHERE nip LIKE ?")
...
class Golongan:
    ...
    def search(nm_golongan):
        try:
            cur.execute(
                f"SELECT * FROM golongan  WHERE nama_golongan LIKE ?", [nm_golongan])
    ...
    def insert():
        try:
            ...
            data = (kd_golongan, nm_golongan, tnj_suami,
                    tnj_anak, uang_mkn, uang_lbr, askes)
            cur.execute(sql, data)
            connect.commit()
            print("\nData Berhasil Di Tambahkan")
            kembali()
        ...
        try:
            for x in cur.fetchall():
                if nm_golongan in x[1]:
                    ...
                    sql = (
                        "UPDATE golongan SET tunjangan_suami = ?, tunjangan_anak = ?, uang_makan = ?, uang_lembur = ? WHERE nama_golongan LIKE ?")
                    data = (tnj_suami, tnj_anak, uang_mkn,
                            uang_lbr, nm_golongan)
                    cur.execute(sql, data)
                    connect.commit()
    ...
    def delete(nm_golongan):
        ...
        try:
            for x in cur.fetchall():
                if nm_golongan in x[1]:
                    cur.execute(
                        f"DELETE FROM golongan WHERE nama_golongan LIKE ?", [nm_golongan])
    ...
    if pilihan == '1':
        ...
        try:
            ...
            if menu == '1':
                ...
                Golongan.update(
                    nm_golongan=input(
                        "Masukan Nama Golongan yg Ingin Di Ubah : ")
                )
            elif menu == '5':
                clear_screen()
                Golongan.delete(
                    nm_golongan=input(
                        "Masukan Nama Golongan yg Ingin Di Hapus : ")
                )
        except:
            clear_screen()
            print("EROR")
