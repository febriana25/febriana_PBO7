...
cur.execute(
    "CREATE TABLE transaksi_brg (no_nota VARCHAR(3), kode_brg VARCHAR(3), qty INT(3), harga INT(30))")
...
def search(kd_brg):
    try:
        cur.execute(
            "SELECT nama_sup ,nama_brg, tanggal ,tempo, qty, harga, total FROM supplier, barang, transaksi_brg, nota WHERE supplier.kode_sup=nota.kode_sup AND barang.kode_brg=transaksi_brg.kode_brg AND nota.no_nota=transaksi_brg.no_nota AND barang.kode_brg=?", [kd_brg])
...
def insert():
    try:
        ...
        total = qty * harga
        cur.execute("INSERT INTO supplier (kode_sup, nama_sup) VALUES (?,?)", [
                    kode_sup, nama_sup])
        connect.commit()
        cur.execute("INSERT INTO barang (kode_brg, nama_brg) VALUES (?,?)", [
                    kode_brg, nama_brg])
        connect.commit()
        cur.execute("INSERT INTO nota (no_nota, tanggal, tempo, total, kode_sup) VALUES (?,?,?,?,?)", [
                    no_nota, tanggal, tempo, total, kode_sup])
        connect.commit()
        cur.execute("INSERT INTO transaksi_brg (no_nota, kode_brg, qty, harga) VALUES (?,?,?,?)", [
...
def update(kd_brg):
    cur.execute(
        "SELECT barang.kode_brg, nota.kode_sup ,nama_sup ,nama_brg, tanggal ,tempo, qty, harga, total FROM supplier, barang, transaksi_brg, nota WHERE supplier.kode_sup=nota.kode_sup AND barang.kode_brg=transaksi_brg.kode_brg AND nota.no_nota=transaksi_brg.no_nota AND barang.kode_brg=?", [kd_brg])
    try:
        for x in cur.fetchall():
            kode_brg = x[0]
            kode_sup = x[1]
            if kd_brg in x[0]:
                ...
                total = qty * harga
                cur.execute("UPDATE transaksi_brg SET qty = ?, harga = ? WHERE kode_brg LIKE ?", [
                            qty, harga, kode_brg])
                connect.commit()
                cur.execute("UPDATE nota SET total = ? WHERE kode_sup LIKE ?", [
                            total, kode_sup])
                connect.commit()
...
def delete(kd_brg):
    ...
    try:
        for x in cur.fetchall():
            ...
            if kd_brg in x[0]:
                cur.execute(
                    "DELETE FROM barang WHERE kode_brg = ?", [kode_brg])
                connect.commit()
                cur.execute(
                    "DELETE FROM supplier WHERE kode_sup = ?", [kode_sup])
                connect.commit()
                cur.execute("DELETE FROM nota WHERE kode_sup = ?", [kode_sup])
                connect.commit()
                cur.execute(
                    "DELETE FROM transaksi_brg WHERE kode_brg = ?", [kode_brg])
                connect.commit()
