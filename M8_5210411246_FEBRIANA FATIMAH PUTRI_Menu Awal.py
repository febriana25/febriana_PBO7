...
def menu_awal():
    while(True):
        print("         Selamat Datang di Perpustakan UTY         ")
        print("---------------------------------------------------")
        ...
        try:
            ...
            try:
                if(int(jumlah_stok[a]) > 0):
                    ...
                    with open(t, "a") as f:
                        f.write(
                            "1. \t\t "+judul_buku[a]+" \t\t "+pengarang[a]+" \n ")

                        jumlah_stok[a] = int(jumlah_stok[a])-1
                        with open("stock.txt", "r+") as f:
                            for i in range(8):
                                f.write(
                                    judul_buku[i]+","+pengarang[i]+","+str(jumlah_stok[i])+"\n")
                                continue
                    ...
                    while loop == True:
                        choice = str(
                            input("Apakah ingin pinjam buku lagi? ketik y untuk ya dan n untuk tidak"))
                        if(choice.upper() == "Y"):
                            count = count+1
                            print("Pilih menu di bawah ini : ")
                            for i in range(len(judul_buku)):
                                print("Masukkan", i,
                                      "untuk meminjam buku", judul_buku[i])
                            a = int(input())
                            if(int(jumlah_stok[a]) > 0):
                                print("Buku tersedia")
                                with open(t, "a") as f:
                                    f.write(str(count) + ". \t\t " +
                                            judul_buku[a]+" \t\t "+pengarang[a]+"\n")
...
def kembalikan_buku():
    ...
    if(stat.upper() == "Y"):
        ...
        with open("stock.txt", "r+") as f:
            for i in range(8):
                f.write(judul_buku[i]+","+pengarang[i] +
                        ","+str(jumlah_stok[i])+"\n")
